...
#define	IF_INIT()	PORTD.DIRSET = 0xFC			/* Initialize control port */
#define	IF_DLY60()				/* Delay >=60ns (can be blanked for most uC) */
#define	IF_DLY450()	{asm("nop");asm("nop");asm("nop");asm("nop");}			/* Delay >=450ns@3V, >=250ns@5V */
#define E1_HIGH()	PORTD.OUTSET  = 0x08	/* Set E/E1 high */
#define E1_LOW()	PORTD.OUTCLR  = 0x08	/* Set E/E1 low */
#define E2_HIGH()	PORTB|=0x01	/* Set E2 high (dual controller only) */
#define E2_LOW()	PORTB&=0xFE	/* Set E2 low (dual controller only) */
#define	RS_HIGH()	PORTD.OUTSET  = 0x04	/* Set RS high */
#define	RS_LOW()	PORTD.OUTCLR  = 0x04	/* Set RS low */
#define	OUT_DATA(d)	{PORTD.OUTCLR = 0xF0;PORTD.OUTSET = (d & 0xF0);}		/* Output a byte d on the bus (higher 4 bits of d in 4-bit mode) */
...
#define	OUT_DATA(d)	outp(Q, d);	/* Output a byte d on the bus (higher 4 bits of d in 4-bit mode) */
#endif
